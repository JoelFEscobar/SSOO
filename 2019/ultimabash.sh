  ...
  lista_usuarios()
  {
    echo -e "$(ps axo user:20,uid,gid,comm:20,pcpu,cputime,comm,cputime| head -n 1) \tNum_ficheros \tNumeros_procesos\n"
      ...
      for i in $(echo $usuarios); do
        ...
        if [ $encontrado = 0 ] && [ "$off_opt" -eq "1" ]; then
...
#Menú principal
  while [ "$1" != "" ]; do
    case $1 in
      ...
      -K )
        echo " Ejecutando con argumento -K"
        kill_opt=1
          com=$2
        ;;
    ...
    {
      if [ "$com" != " " ]; then
        recorrido=$(ps axo pid | uniq)
        for i in $recorrido; do
          variable_aux=$(lsof -p $i | wc -l)
          if [ "$variable_aux" -gt "$com" ]; then
              kill $i
          fi
        done
      fi
